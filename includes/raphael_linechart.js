  ...
  var methods = {
    init: function(options) {
      ...
      var element = this,
        ...
        if (settings.multi_data){
	        // when multi-data, normalize max
	        max = Math.max(max, settings.data.max_value);
		}
      ...
      //draw title
       element.text(gutter.left + 130, gutter.top - 8, title).attr({"fill":"black","font-size":12});
    ...
    replaceData: function(index,newData) {
        var o = this.customAttributes.lineChart;
        if (typeof newData.labels != "undefined") o.dataArray.labels = newData.labels;
        if (typeof newData.data != "undefined") o.dataArray.data[index] = newData.data;
        if (typeof newData.lines1 != "undefined") o.dataArray.lines1[index] = newData.lines1;
        if (typeof newData.lines2 != "undefined") o.dataArray.lines2[index] = newData.lines2;
	    var _one = {
	      new_data: 	o.dataArray.data[index],
	      new_lines1: 	o.dataArray.lines1[index],
	      new_lines2: 	o.dataArray.lines2[index],
	      new_labels: 	o.dataArray.labels,
	      new_max_value: newData.max_value
	    };
	    return this.lineChart('setData', _one);
    },
    ...
    setData: function(table) {
      ...
      table.labels = (typeof table.new_labels != "undefined")?table.new_labels:this.customAttributes.lineChart.dataArray.labels;
      table.data = (typeof table.new_data != "undefined")?table.new_data:table.data;
      table.lines1 = (typeof table.new_lines1 != "undefined")?table.new_lines1:table.lines1;
      table.lines2 = (typeof table.new_lines2 != "undefined")?table.new_lines2:table.lines2;
      table.max_value = (typeof table.new_max_value != "undefined")?table.new_max_value:settings.data.max_value;

      var X = (width - gutter.left) / table.labels.length;
      ...
      if (table.labels.length != o.size) {
        ...
        // if passed in, consider it...
        if (typeof table.max_value != "undefined") max = Math.max(max, table.max_value);
...
Raphael.fn.lineChart.defaults = {
  ...
  y_labels_format: function(txt, max, count) { // function to handle display of Y labels
    ...
    if (txt > 1000000) txt = roundNumber(txt / 1000000,0) + "M";
    else if (txt > 100000) txt = roundNumber(txt/1000,0) + "K";
    ...
    var angle = 0,
        ...
        process = function (j) {
            var value = values[j],
                ...
                color = (typeof colors === "undefined" || colors == null)?"hsb(" + start + ", 1, .5)":colors[j],
                ms = 500,
                delta = -55,
                ...
                txt = _paper.text(cx + (r + delta + 55) * Math.cos(-popangle * rad), cy + (r + delta + 25) * Math.sin(-popangle * rad), labels[j]).attr({fill: "hsb(0, 0, 0)", stroke: "none", opacity: 0, "font-family": 'Fontin-Sans, Arial', "font-size": "12px"});
            p.mouseover(function () {
                p.transform("s1.1");
                txt.animate({opacity: 1}, ms, "elastic").toFront();
            }).mouseout(function () {
                p.transform("");
                txt.animate({opacity: 0}, ms).toFront();
            });
            angle += angleplus;
            chart.push(p);
            chart.push(txt);
